...
void screengrab_thread(void* user) {
  ...
  ThreadedImageSaver* gsp = static_cast<ThreadedImageSaver*>(user);
  ThreadedImageSaver& gs = *gsp;
...
bool ThreadedImageSaver::setup(int winW, int winH, int bufferSize) {
...
bool ThreadedImageSaver::grab(std::string filename) {
...
bool ThreadedImageSaver::grabFbo(std::string filename, ofFbo fbo ) {
...
//    }
    ...
    uv_mutex_lock(&mutex);
    {
        fbo.readToPixels( gd->img.getPixelsRef() );
