...
#define SGS_STATE_NONE 0 /* default state of ThreadedImageSaver */
...
class ThreadedImageSaver {
 public:
  ...
  bool grab(std::string filename);          /* will grab the data in the caller thread, and saves the data to file in a separate thread */
    bool grabFbo(std::string filename, ofFbo fbo );
 public:
...
inline GrabData* ThreadedImageSaver::getFreeGrabData() {
